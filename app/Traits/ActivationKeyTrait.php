...
trait ActivationKeyTrait
{
    ...
    public function createActivationKeyAndNotify(User $user)
    {
        ...
        if ($user->activated) {
            return redirect()->route('front.home')
                ->with('message', 'Akun ini sudah aktif.')
                ->with('status', 'success');
        }
    ...
    public function processActivationKey(ActivationKey $activationKey){
        ...
        if (empty($userToActivate)) {
            return redirect()->route('front.home')
                ->with('message', 'Kami tidak dapat menemukan pengguna dengan kode aktivasi ini! Silakan mendaftar untuk mendapatkan kunci yang valid!')
                ->with('status', 'success')
                ->with('type','success');
        }
